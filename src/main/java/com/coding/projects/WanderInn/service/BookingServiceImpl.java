...
import com.coding.projects.WanderInn.entity.*;
import com.coding.projects.WanderInn.repository.*;
...
import com.coding.projects.WanderInn.exception.UnAuthorisedException;
import com.coding.projects.WanderInn.strategy.PricingService;
...
public class BookingServiceImpl implements BookingService{
    ...
    public void capturePayment(Event event) {
        ...
        // 1) Get Session ID from raw JSON
        EventDataObjectDeserializer data = event.getDataObjectDeserializer();
        String raw = data.getRawJson();
        String sessionId = JsonParser.parseString(raw).getAsJsonObject().get("id").getAsString();

        // 2) Retrieve Session (handle checked exception)
        Session session;
        try {
            session = Session.retrieve(sessionId);
        } catch (StripeException se) {
            log.error("Stripe retrieve failed for session {}", sessionId, se);
            // Re-throw unchecked so the controller returns non-2xx and Stripe retries
            throw new RuntimeException(se);
        }

        String paymentStatus = session.getPaymentStatus();
        if (!"paid".equalsIgnoreCase(paymentStatus)) {
            log.info("Session {} not paid yet; skipping (status={})", sessionId, paymentStatus);
            return;
        }
